...
Each word's length is greater than 0 and not exceeding maxWidth.
...
class Solution:
    def fullJustify(self, words: List[str], maxWidth: int) -> List[str]:
        res: List[str] = []
        line: List[str] = []
        line_len = 0  # sum of lengths of words in 'line' (no spaces)

        i = 0
        n = len(words)
        while i < n:
            w = words[i]
            # If we add w, total width = line_len + len(line) (spaces) + len(w)
            if line_len + len(line) + len(w) <= maxWidth:
                ...
                if len(line) == 1:
                    ...
                    # Leftmost 'extra' gaps get one more space
                    parts = []
                    for j in range(gaps):
                        parts.append(line[j])
                        parts.append(" " * (base + (1 if j < extra else 0)))
                    parts.append(line[-1])
